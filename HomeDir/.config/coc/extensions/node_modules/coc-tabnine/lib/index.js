...
const child_process_1 = __importDefault(__webpack_require__(27));
const semver_1 = __importDefault(__webpack_require__(28));
...
const await_semaphore_1 = __webpack_require__(31);
const mkdirp_1 = __importDefault(__webpack_require__(32));
const download_1 = __importDefault(__webpack_require__(33));
...
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const configuration = coc_nvim_1.workspace.getConfiguration('tabnine');
        const tabNine = new TabNine(context.storagePath);
        let binaryRoot = path_1.default.join(context.storagePath, 'binaries');
        if (!fs_1.default.existsSync(binaryRoot)) {
            mkdirp_1.default.sync(binaryRoot);
        }
        yield TabNine.installTabNine(binaryRoot);
        let priority = configuration.get('priority', undefined);
        let disable_filetyps = configuration.get('disable_filetyps', []);
        let limit = configuration.get('limit', 10);
        subscriptions.push(coc_nvim_1.commands.registerCommand('tabnine.openConfig', () => __awaiter(this, void 0, void 0, function* () {
            const res = yield tabNine.request("1.0.7", {
                Autocomplete: {
                    filename: '1',
            ...
            if (!res.results || res.results.length < 0) {
                coc_nvim_1.workspace.showMessage('TabNine::config_dir return empty result', 'error');
                return;
            }
            let folder = res.results[0].new_prefix;
            let file = path_1.default.join(folder, 'tabnine_config.json');
            yield coc_nvim_1.workspace.openResource(coc_nvim_1.Uri.file(file).toString());
        })));
        subscriptions.push(coc_nvim_1.languages.registerCompletionItemProvider('tabnine', configuration.get('shortcut', 'TN'), null, {
            provideCompletionItems(document, position, token, context) {
                return __awaiter(this, void 0, void 0, function* () {
                    if (disable_filetyps.indexOf(document.languageId) !== -1)
                        return null;
                    let { option } = context;
                    try {
                        ...
                        const before = document.getText(vscode_languageserver_protocol_1.Range.create(before_start, position));
                        const after = document.getText(vscode_languageserver_protocol_1.Range.create(position, after_end));
        ...
        function makeCompletionItem(args) {
            ...
            item.textEdit = vscode_languageserver_protocol_1.TextEdit.replace(vscode_languageserver_protocol_1.Range.create(start, end), newText);
            ...
            if (item.detail == null && item.insertTextFormat != vscode_languageserver_protocol_1.InsertTextFormat.Snippet) {
                item.data = item.data || {};
                item.data.dup = 0;
            }
            else if (args.index == 0 && item.insertTextFormat == vscode_languageserver_protocol_1.InsertTextFormat.Snippet) {
                item.preselect = true;
            }
            if (args.entry.kind) {
                item.kind = args.entry.kind;
            }
            else if (item.insertTextFormat == vscode_languageserver_protocol_1.InsertTextFormat.Snippet) {
                item.kind = vscode_languageserver_protocol_1.CompletionItemKind.Snippet;
            }
            let pre = args.document.getText(vscode_languageserver_protocol_1.Range.create(args.position.line, 0, args.position.line, args.position.character));
        ...
        function completionIsAllowed(document, position) {
            ...
            for (const r of disable_line_regex) {
                if (line === undefined) {
                    line = document.getText(vscode_languageserver_protocol_1.Range.create({ line: position.line, character: 0 }, { line: position.line, character: 500 }));
                }
...
class TabNine {
    ...
    request(version, any_request) {
        return __awaiter(this, void 0, void 0, function* () {
            const release = yield this.mutex.acquire();
            try {
                return yield this.requestUnlocked(version, any_request);
            }
            finally {
                release();
            }
    ...
    restartChild() {
        ...
        const binary_root = path_1.default.join(this.storagePath, "binaries");
        const command = TabNine.getBinaryPath(binary_root);
        this.proc = child_process_1.default.spawn(command, args);
    ...
    static installTabNine(root) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let version = yield coc_nvim_1.fetch('https://update.tabnine.com/version');
            ...
            item.show();
            try {
                let dest = path_1.default.join(root, `${version}/${arch}-${suffix}`);
                yield download_1.default(url, dest, percent => {
                    item.text = `Downloading TabNine ${(percent * 100).toFixed(0)}%`;
                });
                fs_1.default.chmodSync(dest, 0o755);
            }
            catch (e) {
                coc_nvim_1.workspace.showMessage(`Download error ${e.message}`, 'error');
            }
            item.dispose();
        });
    }
    static getBinaryPath(root) {
        ...
        const versions = fs_1.default.readdirSync(root);
    ...
    static cmpSemver(a, b) {
        const a_valid = semver_1.default.valid(a);
        const b_valid = semver_1.default.valid(b);
        if (a_valid && b_valid) {
            return semver_1.default.rcompare(a, b);
        }
        else if (a_valid) {
            return -1;
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
const follow_redirects_1 = __webpack_require__(34);
const fs_1 = __importDefault(__webpack_require__(29));
const mkdirp_1 = __importDefault(__webpack_require__(32));
const path_1 = __importDefault(__webpack_require__(12));
const tunnel_1 = __importDefault(__webpack_require__(49));
const url_1 = __webpack_require__(35);
const coc_nvim_1 = __webpack_require__(1);
...
function download(url, dest, onProgress) {
    ...
    let folder = path_1.default.dirname(dest);
    ...
    return new Promise((resolve, reject) => {
        const req = mod.request(opts, res => {
            ...
            let stream = res.pipe(fs_1.default.createWriteStream(dest));
